    ...
    for y in range(1,len(Years)):
        ...
        for theta in thetas:
            ...
            while (wave == 0) or (len(new_failed_names) != 0):
                ...
                if len(new_failed_names) != 0 and check == 0 and 'a' in new_failed_names:
                	check = 1
                	tempo = theta
...
gdp1 = 100
gdp2 = 150
Graph_plot = pd.DataFrame(index = range(2,noOfCountries), columns = range( 80,140, 20))

for gdp2 in range( 90,200, 10):
    Graph_vals = []
    print(gdp2)
    for n in range(2,noOfCountries):
       # print(n)
        index = createIndex(n)
        #print(index)
        ...
        #print(n, tempo)
        Graph_vals.append(tempo)

#CascadeResult.to_csv('Res_moreGDP_moreTrade.csv')
#print(Graph_vals)
#Graph_plot = pd.Series(Graph_vals, index = range(2,noOfCountries))
    Graph_plot[gdp2] = Graph_vals
Graph_plot.to_csv('Plots_sameTrade.csv')
